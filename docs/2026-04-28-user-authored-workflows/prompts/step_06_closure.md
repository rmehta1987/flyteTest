...
- [x] 2026-04-28 `mcp_tools.py`: added `vc_custom_filter` flat tool for
  ...
  params for Slurm); added `vc_germline_filtered` flat tool for composed workflow.
- [x] 2026-04-28 `workflows/variant_calling.py`: added
  `germline_short_variant_discovery_filtered` composed workflow — applies
...
- [x] 2026-04-28 `registry/_variant_calling.py`: added `RegistryEntry` for
  `germline_short_variant_discovery_filtered` (`category="workflow"`,
...
- [x] 2026-04-28 Tests: `VcCustomFilterFlatToolTests`,
  `VcGermlineFilteredFlatToolTests` in `test_mcp_tools.py`;
  `GermlineFilteredWorkflowRegistryTests` in `test_variant_calling.py`.
