...
| Generic biology asset adoption | Current | Generic biology asset names are now the preferred internal asset surface in the current BRAKER3, STAR, PASA, TransDecoder, protein-evidence, annotation-refinement, and consensus-annotation-compatible emitters. Milestone 23 introduced `CodingPredictionResult` (generic sibling of `TransDecoderPredictionResult`). Milestone 24 introduced `ProteinAlignmentChunkResult` (generic sibling of `ExonerateChunkAlignmentResult`). Milestone 25 introduced `AnnotationRefinementResultBundle` (generic sibling of `PasaGeneModelUpdateResultBundle`). Milestone 26 introduced `ConsensusAnnotationResultBundle` as the biology-facing generic sibling of `EvmConsensusResultBundle`; new manifests emit both `consensus_annotation_result_bundle` (preferred) and `evm_consensus_result_bundle` (legacy) keys, and historical manifests remain replayable unchanged. The 7 internal EVM computation types remain EVM-named. Planner adapters prefer the generic keys when the semantic meaning is known, and legacy alias replay remains available for historical manifests. No further family-scoped generic-sibling candidates remain in the current audit (`docs/realtime_refactor_plans/2026-04-10-post-m17-asset-surface-follow-up-audit.md`). |
| Slurm retry and resubmission policy | Current | `src/flytetest/spec_executor.py` records a conservative `SlurmFailureClassification` plus explicit `SlurmRetryPolicy` in each durable run record. `retry_slurm_job` resubmits from the original frozen recipe unchanged and links each child attempt back to its parent run record. `TIMEOUT` and `OUT_OF_MEMORY` failures (classified as `resource_exhaustion`) can be retried in-place via `retry_slurm_job` by passing `resource_overrides` with new values for `cpu`, `memory`, `walltime`, `queue`, `account`, or `gpu`; the child run record stores both the effective `resource_spec` and the caller-supplied `resource_overrides` for audit. `DEADLINE` failures are excluded from escalation and always require a new `prepare_run_recipe` call. |
...
| AGAT post-processing | Current | The statistics, conversion, and deterministic cleanup slices after EggNOG functional annotation are wired through task and workflow boundaries. |
| table2asn / NCBI submission | Current (M21c) | `annotation_postprocess_table2asn` workflow and `table2asn_submission` task consume the AGAT cleanup bundle and produce an NCBI `.sqn` submission file via `table2asn -M n -J -c w -euk -gaps-min 10 -l proximity-ligation`. The task is workflow-surface only (not exposed via `run_task`). |
...
| Ad hoc task execution | Current (M21) | `run_task()` now dispatches all four eligibility-gated tasks (`exonerate_align_chunk`, `busco_assess_proteins`, `fastqc`, `gffread_proteins`) through a `TASK_PARAMETERS` dispatch dict derived from `SHOWCASE_TARGETS`; unknown names, missing required inputs, and extra keys all return structured `supported=False` declines. Eligibility is controlled by explicit `ShowcaseTarget(category="task")` entries — not by `synthesis_eligible`. |
| Binding discovery | Current (M21) | `list_available_bindings(task_name, search_root)` performs a depth-3 heuristic file-extension scan (FASTA/GFF3/FASTQ patterns per parameter) and returns candidate file paths for file-typed parameters and a scalar-entry hint for string/int parameters. |
| Run dashboard | Current (M21) | `get_run_summary(run_dir, limit)` scans persisted `slurm_run_record.json` and `local_run_record.json` files offline, groups runs by state, and returns `total_scanned`, `by_state`, and a capped `recent` list with `kind`/`state`/`workflow_name` per entry. `inspect_run_result(run_record_path)` loads one record and returns a structured summary with no scheduler calls. |
| Job log fetching | Current (M21b) | `fetch_job_log(log_path, tail_lines)` returns the last N lines of a Slurm stdout or stderr log. The path is validated against the default run directory using the same `_read_text_tail` path-traversal guard as `monitor_slurm_job`; paths outside the run directory return `supported=False`. |
| Job polling / wait-for-completion | Current (M21b) | `wait_for_slurm_job(run_record_path, timeout_s, poll_interval_s)` polls `monitor_slurm_job` at configurable intervals until the job reaches a terminal scheduler state or the timeout expires. Returns the standard `monitor_slurm_job` payload plus a `timed_out` boolean. Poll interval is floored at 5 seconds. |
...
| Resource-aware execution planning | Current | The recipe-backed planner freezes a selected execution profile plus structured `ResourceSpec` and optional `RuntimeImageSpec` metadata into saved `BindingPlan` artifacts and local or Slurm submission results. Registry workflow metadata exposes two advisory resource tables per workflow: local defaults under `execution_defaults["resources"]` (cpu, memory, execution_class) and Slurm-specific hints under `execution_defaults["slurm_resource_hints"]` (cpu, memory, walltime) sized for a typical small-to-medium eukaryote genome. Both are starting-point suggestions; any explicit `resource_request` from the caller takes precedence. Slurm lifecycle reconciliation records the observed scheduler state alongside these frozen settings. |
...
| Storage-native durable asset return | Current (M20b) | After every successful local execution, `LocalWorkflowSpecExecutor` writes a `durable_asset_index.json` sidecar alongside `local_run_record.json`.  Each Path-valued output produces one `DurableAssetRef` entry (fields: `run_id`, `workflow_name`, `output_name`, `node_name`, `asset_path`, `manifest_path`, `created_at`, `run_record_path`).  `LocalManifestAssetResolver.resolve()` accepts an optional `durable_index` parameter; when a manifest source path is missing and a matching ref is found, an explicit limitation message citing `run_id` and `output_name` is surfaced rather than silently failing.  Legacy run directories without an index load cleanly. |
