...
| Step | Name | Status |
|---|---|---|
| 01 | Bug fixes (bundles, stage script, registry module_loads) | [x] |
...
| 05 | Closure (tests, CHANGELOG, merge) | [x] |
...
- [x] Registry: `pre_call_coverage_qc` module_loads includes `"multiqc"` not `"gatk"` alone
- [x] Registry: `post_call_qc_summary` module_loads has `"bcftools"` and `"multiqc"`
- [x] Registry: `annotate_variants_snpeff` module_loads has `"snpeff"` not `"gatk"`
