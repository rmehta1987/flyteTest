...
as the future planning and input-mapping layer. It deliberately lives outside
`src/flytetest/types/` so these higher-level biology types stay separate from
the existing lower-level local-path asset catalog used by manifests and result
collectors.
"""
...
def _serialize_value(value: Any) -> Any:
    ...
    This helper is intentionally small and recursive so every planner dataclass
    can share one predictable serialization path for `Path`, tuple, and nested
    dataclass members.
    """
...
def _deserialize_value(annotation: Any, value: Any) -> Any:
    ...
    The function mirrors `_serialize_value(...)` so planner dataclasses can
    round-trip through dict payloads while preserving nested dataclasses,
    immutable tuples, and `Path` members.
    """
    if value is None:
        return None
...
class PlannerSerializable:
    ...
    `to_dict()` and `from_dict()` in each type. That keeps the planner-facing
    data shape small, consistent, and easy to rebuild from manifests or future
    ...
    def from_dict(cls: type[_PlannerSerializableT], payload: Mapping[str, Any]) -> _PlannerSerializableT:
        ...
        Unknown fields are ignored intentionally so adapters can pass through
        additive payloads without breaking older planner-type readers.
        """
