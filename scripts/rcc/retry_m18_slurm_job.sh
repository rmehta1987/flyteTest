...
DEFAULT_POINTER="$REPO_ROOT/.runtime/runs/latest_m18_retry_smoke_run_record.txt"
RUN_RECORD_PATH="${1:-}"
...
if [[ -z "$RUN_RECORD_PATH" ]]; then
  ...
  echo "or set $DEFAULT_POINTER by running the Milestone 18 retry-smoke record wrapper first." >&2
  exit 1
fi
